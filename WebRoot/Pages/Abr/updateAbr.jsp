...
<div class="content-box">
			<form name="myForm" method="post"  action="abrAction.do?method=updateAbr" >
						   <div class="source">
	...
	function showAbrInfo(__id){
		...
		$.post("abrAction.do?method=getAbrInfoById",{id:__id},function(data){
			...
			for(var i=0; i<inputList.length; i++){
				abrInfoHtml += '<p><label>输入地址：</label><input name="inputs" type="hidden" value="'+inputList[i].addr+'_'+inputList[i].bandwidth+'" /> <input type="text" value="'+inputList[i].addr+'"/>'+
					'<label>码率(kbps)：</label><input type="text" value="'+inputList[i].bandwidth+'" /></p>';
			}
			abrInfoHtml += '<p><label>输出地址：</label><input type="text" value="'+abrInfo.output+'" /></p>';
